...
int MCTS::GreedyUCB(VNODE* vnode, bool ucb) const
{
	...
	// cout << (ucb ? "----- PLANING -----" : "----- EXECUTION -----") << endl;
	std::vector<vector<double>> q_list;

	for (int action = 0; action < Simulator.GetNumActions(); action++)
	{
		QNODE& qnode = vnode->Child(action);
		vector<double> q = qnode.Value.GetValue();
		int n = qnode.Value.GetCount();
		double bonus = FastUCB(N, n, logN);
		// cout << "q = " << q << endl;
	...
	vector<double> pi_star = operations_research::solve(q_list, Params.ImportanceWeight);
	// cout << "pi star = " << pi_star << endl;
	// sample an action from the distribution
	return sample(pi_star);
}
